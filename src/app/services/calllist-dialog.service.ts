...
export class CallListDialogService {
  ...
  showcallListDialog(row: any): Promise<void> {
    ...
    return this.apiService.calllistList<any>(request)
      .then(res => {
        const calllistList = res?.data || [];
        this.openDialog(row, calllistList);
      })
      .catch(err => {
  ...
  private openDialog(row: any, calllistList: any[]) {

  let html = '';

  if (calllistList && calllistList.length) {
